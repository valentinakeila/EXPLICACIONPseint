Proceso trabajofinal
	...
	Dimension cantidadVacunas[6,2] // contadores de stock de vacuna
	
	Definir tamaño Como Entero
	tamaño = 40
	
	Dimension usuario[tamaño,7] //Array que contiene los datos del usuario 7 son los datos del usuario
	...
	Repetir
		...
		SiNo
			...
			Repetir
				...
				opcionDia = opcionDia - 1 // porque el array empieza en 0 es como si todo estuviera mas arriba entonces se le resta
				
				// para escribir los horarios de cada dia si estos son diferentes a ocupados
				...
				Repetir
					// esta bandera sirve para que no se elijan horarios ocupados invisibles
					banderaOcupado = 0
					Repetir
						Leer opcionHorario
					...
					Para i <- 0 Hasta 8 Con Paso 1 Hacer
						...
						Si opcionDia == i Entonces
							Si turnos[i,opcionHorario] == "Ocupado" Entonces
								banderaOcupado = 1 // si esta ocupado es 1
							SiNo
								turnos[i,opcionHorario] = "Ocupado" // sino estaba ocupado se ocupa en el repetir la bandera empieza en cero
							Fin Si
						Fin Si
					Fin Para
					...
					Fin Si
				Hasta Que banderaOcupado <> 1 // porque si vale 1 el turno estaba ocupado
				
				
				cantidadTurnos[opcionDia,1] = ConvertirATexto(ConvertirANumero(cantidadTurnos[opcionDia,1]) + 1) //Opcion 5 - Contador de la cantidad de turnos
...
Para i <- 0 Hasta 5 Con Paso 1 Hacer
	Si ConvertirANumero(vacunas[i,1]) > 0 Entonces // si vacunas[i,1] que es el stock es mayor a 0 significa que hay y muestra las que hay, se convierte a numero pq las vacunas son caracter
...
Si bandera2 <> 6 Entonces // Si sigue habiendo stock de vacunas
	...
	vacunas[vacunaElegida,1] = ConvertirATexto(ConvertirANumero(vacunas[vacunaElegida,1]) - 1) // se resta stock
	cantidadVacunas[vacunaElegida,1] = ConvertirATexto(ConvertirANumero(cantidadVacunas[vacunaElegida,1]) + 1) // contador de cada vacuna se ve en la funcion de listado
	// se convierte a texto de nuevo porque al haberlo convertido en numero para hacer la operacion no podes guardarlo en el array que esta definido como caracter
	...
	Para i <- 0 Hasta tamaño - 1 Con Paso 1 Hacer
		Si usuario[i,6] <> "Ocupado" Entonces // Si la casilla 6 vale "Ocupado" se pasa al siguiente espacio para no sobreescribir un usuario ya existente
			...
			usuario[i,2] = ConvertirATexto(edadIngresada) // porque usuario es un array de caracter y edadingresada es un entero por eso se convierte a texto
			Segun vacunaElegida Hacer // segun lo que valiera vacuna elegida se guarda en la tercer columna usamos el segun para guardar la vacuna que eligio
				0:
					usuario[i,3] = "Neumococo conjugada"
				1:/ dia esta guardado en forma numerica  y *segun* su valor escribe un dia u otro
			...
			Fin Segun
			usuario[i,4] = ConvertirATexto(opcionDia) //opcion dia es entero acordarse que el array usuario es caracter por eso pusimos convertir a texto
			...
			Segun ConvertirANumero(usuario[i,4]) + 1 Hacer // dia esta guardado en forma numerica  y *segun* su valor escribe un dia u otro
				1:
					Escribir "Lunes"
				2:
					Escribir "Martes"
			...
			Escribir "Horario: " Sin Saltar // horario esta guardado en forma numerica  y *segun* su valor escribe un horario u otro
			...
			i = tamaño + 1 // Si se registra el usuario, i vale tamaño + 1 para que deje de iterar sobre los espacios no ocupados para que no siga repitiendo el para y termine una vez guardado el usuario
...
SubProceso ordenar(usuario,tamaño,cantidadTurnos)
	...
	Segun Minusculas(opcion4) Hacer
		"a": // ordenamiento por edad
			Para i<-0 Hasta tamaño -2 Con Paso 1 Hacer // -2 para que no se compare consigo misma
				Para j<-i+1 Hasta tamaño -1 Con paso 1 Hacer // j vale i+1 para que no se compare consigo misma
					si ConvertirANumero(usuario[i,2]) > ConvertirANumero(usuario[j,2]) Entonces // Si la columna de la edad es mayor a la de la linea siguiente, se intercambian
						Para k<-0 Hasta 7-1 Con Paso 1 Hacer // se usa k para que se intercambien todas las columnas
...
SubProceso listadoTurnos(cantidadTurnos,cantidadVacunas)
	...
	Segun Minusculas(opcion1) Hacer // Se convierte a minusculas por si ingresa una mayuscula
		...
		"b":
			...
			Para i <- 0 Hasta 5 Con Paso 1 Hacer // nombre vacuna
				Para j <- 0 Hasta 1 Con Paso 1 Hacer // se escribe la columna donde estan la cantidad de vacuna
